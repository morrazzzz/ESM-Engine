...
constexpr float sun_accum_limit = 100.f;
...
void CRenderTarget::accum_direct_cascade(const light& sun_light, u32 sub_phase, Fmatrix& xform, Fmatrix& xform_prev, float fBias)
{
	...
	if (RImplementation.o.sunfilter) {
		accum_direct_f(sun_light, sub_phase);
		return;
	}
	...
	L_clr.set(sun_light.color.r, sun_light.color.g, sun_light.color.b);
	L_spec = u_diffuse2s(L_clr);
	Device.mView.transform_dir(L_dir, sun_light.direction);
	L_dir.normalize();
...
//		float			fBias				= (SE_SUN_NEAR==sub_phase)?(-ps_r2_sun_depth_near_bias):ps_r2_sun_depth_far_bias;
		...
		Fmatrix				m_shadow;
		{
			Fmatrix			xf_project;		xf_project.mul(m_TexelAdjust, sun_light.X.D.combine);
		...
		Fmatrix				m_clouds_shadow;
		{
			...
			Fmatrix			m_xform;
			Fvector			direction = sun_light.direction;
		...
		if (RImplementation.o.advancedpp && (ps_r_sun_shafts > 0) && sub_phase == SE_SUN_FAR)
			accum_direct_volumetric(sun_light, sub_phase, Offset, m_shadow);
	}
}
...
//	if (sub_phase!=SE_SUN_FAR) return;
	...
	{
		LPCSTR pszSMapName = r2_RT_smap_depth;
