...
{
	VERIFY(L->flags.type == IRender_Light::REFLECTED);
	phase_accumulator				();
	...
	{
		RCache.set_Shader(shader, 0, &*g_accum_point);
		// Constants
      ...
      else // checked Holger
      {
		   ...
		   {
			   RCache.set_Shader(shader_msaa[0], 0, &*g_accum_point);
         ...
         else // checked Holger
         {
			 ...
			 {
				 RCache.set_Shader(shader_msaa[i], 0, &*g_accum_point);
				 RCache.set_Stencil(TRUE, D3DCMP_EQUAL, 0x81, 0x81, 0x00);
				 if (bIntersect)	RCache.set_CullMode(CULL_CW);		// back
				 else			RCache.set_CullMode(CULL_CCW);		// front
