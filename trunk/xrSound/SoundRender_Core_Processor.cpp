...
float CSoundRender_Core::get_occlusion_to( const Fvector& hear_pt, const Fvector& snd_pt, float dispersion )
{
	...
	if (0!=geom_SOM){
		...
		u32 r_cnt				= static_cast<u32>(geom_DB.r_count());
...
float CSoundRender_Core::get_occlusion(Fvector& P, float R, Fvector* occ)
{
	...
	if (0!=geom_MODEL){
		...
		if (bNeedFullTest){
			...
			if (0!=geom_DB.r_count()){ 
				// cache polygon
				const CDB::RESULT*	R_GeomDB = geom_DB.r_begin		();
#endif            
				const CDB::TRI&		T = geom_MODEL->get_tris	() [ R_GeomDB->id ];
	...
	if (0!=geom_SOM){
		...
		if (0!=r_cnt){
			for (u32 k=0; k<r_cnt; k++){
				CDB::RESULT* R_Result	 = _B+k;
				occ_value		*= *(float*)&R_Result->dummy;
			}
		}
	}
	return occ_value;
