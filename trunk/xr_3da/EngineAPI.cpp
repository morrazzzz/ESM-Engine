...
void CEngineAPI::CreateRendererList()
{
	...
	constexpr int CountSupportRender = 6;
	...
	_tmp.reserve(CountSupportRender);

	for (u32 i = 0; i < CountSupportRender; ++i)
	{
		LPCSTR val = nullptr;
		...
		if (val)
			_tmp.emplace_back(val);
    }

	vid_quality_token = xr_alloc<xr_token>(_tmp.size() + 1);

	vid_quality_token[_tmp.size()].id = -1;
	vid_quality_token[_tmp.size()].name = NULL;
