...
void CBaseMonster::HitEntity(const CEntity *pEntity, float fDamage, float impulse, Fvector &dir)
{
	...
	if (EnemyMan.get_enemy() == pEntity) {
		...
		u16 bone = (smart_cast<IKinematics*>(pEntityNC->Visual())->LL_GetBoneRoot());
		AddHitObject(fDamage, hit_dir, pEntityNC->ID(), ID(), ID(), bone, position_in_bone_space, impulse, ALife::eHitTypeWound, 0.0f);
...
{
	AddHitObject(value, Fvector().set(0.f, 1.f, 0.f), object->ID(), ID(), ID(), BI_NONE, Fvector().set(0.f, 0.f, 0.f), 0.0f, ALife::eHitTypeTelepatic, 0.0f);
}

void CBaseMonster::Hit_Wound(CObject *object, float value, const Fvector &dir, float impulse) 
{
	u16 bone = smart_cast<IKinematics*>(object->Visual())->LL_GetBoneRoot();
	AddHitObject(value, dir, object->ID(), ID(), ID(), bone, Fvector().set(0.f, 0.f, 0.f), impulse, ALife::eHitTypeWound, 0.0f);
}
