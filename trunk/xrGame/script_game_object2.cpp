...
void CScriptGameObject::Hit(CScriptHit *tpLuaHit)
{
	...
	if (xr_strlen(tLuaHit.m_caBoneName))
		bone = (V->LL_BoneID(tLuaHit.m_caBoneName));
	//	P.w_float		(tLuaHit.m_fImpulse);
	ALife::EHitType typeHit = (ALife::EHitType)(tLuaHit.m_tHitType);					//	P.w_u16			(u16(tLuaHit.m_tHitType));

	AddHitObject(tLuaHit.m_fPower, tLuaHit.m_tDirection, object().ID(), tLuaHit.m_tpDraftsman->ID(), 0, bone,
		Fvector().set(0, 0, 0), tLuaHit.m_fImpulse, typeHit, 0.0f);
}
