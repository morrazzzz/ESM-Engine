...
void player_hud_motion_container::LoadingMotions(IKinematicsAnimated* model, const shared_str& sect)
{
	...
	auto loading = [](LPCSTR anim) -> bool { 
		return strstr(anim, "anm_") == anim || strstr(anim, "anim_") == anim; 
		};

	for(;_b!=_e;++_b)
	{
		if(loading(_b->first.c_str()))
		{
...
//					Msg(" alias=[%s] base=[%s] name=[%s]",pm->m_alias_name.c_str(), pm->m_base_name.c_str(), buff);
			...
			R_ASSERT2(pm->m_animations.size(), make_string("Motion not found [%s] in [%s]", pm->m_base_name.c_str(), sect.c_str()));
		}
	}
}
...
void attachable_hud_item::update(bool bForce)
{
	...
	if(HandsModeHudItem && !!m_measures.m_prop_flags.test(hud_item_measures::e_16x9_mode_now)!=is_16x9)
		m_measures.LoadHands(m_sect_name, m_model);
	...
	m_parent->calc_transform		(m_attach_place_idx, m_attach_offset, m_item_transform, HandsModeHudItem);
...
void attachable_hud_item::RenderAttachableHud(IRenderable* object)
{
	::Render->add_Visual(object, m_model->dcast_RenderVisual(), &m_item_transform, true);
	debug_draw_firedeps();
...
void hud_item_measures::LoadHands(const shared_str& sect_name, IKinematics* K)
{
	bool is_16x9 = UI().is_widescreen();
	string64	_prefix;
...
void hud_item_measures::LoadNotHands(const shared_str& sect_name, IKinematics* K)
{
	bool is_16x9 = UI().is_widescreen();

	for (int i = 0; i < 2; i++)
	{
		m_hands_attach[i].set(0, 0, 0);
		m_hands_offset[i][0].set(0, 0, 0);
	}
	...
	m_prop_flags.set(e_fire_point2, m_prop_flags.test(e_fire_point));
	if (m_prop_flags.test(e_fire_point2))
	{
		m_fire_bone2 = m_fire_bone;

		if (pSettings->line_exist(sect_name, "fire_point2"))
			m_fire_point2_offset = pSettings->r_fvector3(sect_name, "fire_point2");
		else
			m_fire_point2_offset = m_fire_point_offset;
	}
	else
		m_fire_point2_offset.set(0, 0, 0);

	m_prop_flags.set(e_shell_point, m_prop_flags.test(e_fire_point) && pSettings->line_exist(sect_name, "shell_particles"));
	if (m_prop_flags.test(e_shell_point))
	{
	...
	if (pSettings->line_exist(sect_name, "zoom_offset"))
	{
		m_hands_offset[0][1] = pSettings->r_fvector3(sect_name, "zoom_offset");

		m_hands_offset[1][1].x = pSettings->r_float(sect_name, "zoom_rotate_x");
		m_hands_offset[1][1].y = pSettings->r_float(sect_name, "zoom_rotate_y");
	}

	if (pSettings->line_exist(sect_name, "grenade_zoom_offset"))
	{
		m_hands_offset[0][2] = pSettings->r_fvector3(sect_name, "grenade_zoom_offset");

		m_hands_offset[1][2].x = pSettings->r_float(sect_name, "grenade_zoom_rotate_x");
		m_hands_offset[1][2].y = pSettings->r_float(sect_name, "grenade_zoom_rotate_y");
	}
...
void attachable_hud_item::LoadingHudItem(const shared_str& sect_name, IKinematicsAnimated* hands)
{
	m_sect_name	= sect_name;

	//HandsModeHudItem = hands != nullptr;

    shared_str visual_name;
	if (hands && pSettings->line_exist(sect_name, "item_visual"))
	{
		visual_name = pSettings->r_string(sect_name, "item_visual");
		m_attach_place_idx = pSettings->r_u16(sect_name, "attach_place_idx");
		HandsModeHudItem = true;
	}
	else
	{
		visual_name = pSettings->r_string(sect_name, "visual");
		m_attach_place_idx = 0;
		HandsModeHudItem = false;
	}


	// Visual
	//const shared_str& visual_name = pSettings->r_string(sect_name, HandsModeHudItem ? "item_visual" : "visual");
	m_model = smart_cast<IKinematics*>(::Render->model_Create(visual_name.c_str()));

	if (HandsModeHudItem)
	{
		m_measures.LoadHands(sect_name, m_model);

		m_hand_motions.LoadingMotions(hands, sect_name);
	}
	else
	{
		m_measures.LoadNotHands(sect_name, m_model);

		IKinematicsAnimated* animatedHudItem = m_model->dcast_PKinematicsAnimated();
		m_hand_motions.LoadingMotions(animatedHudItem, sect_name);
	}
}
...
u32 attachable_hud_item::anim_play(const shared_str& anm_name_b, BOOL bMixIn, const CMotionDef*& md, u8& rnd_idx, bool& SupportMotionsMark, player_hud_motion* anm_play)
{
	...
	if (!anm_play)
	{
		...
		anm = m_hand_motions.find_motion(anim_name_r);
...
void player_hud::LoadActorHud(const shared_str& player_hud_sect)
{
...
void player_hud::LoadDefaultActorHudIfExist()
{
	if (!pSettings->section_exist("actor_hud_05"))
		return;

	LoadActorHud("actor_hud_05");
}
...
void player_hud::render_hud(IRenderable* root_object)
{
	...
	if (m_model && (m_attached_items[0] && m_attached_items[0]->HandsModeHudItem || m_attached_items[1]))
		::Render->add_Visual(root_object, m_model->dcast_RenderVisual(), &m_transform, true);
	
	if(m_attached_items[0])
		m_attached_items[0]->RenderAttachableHud(root_object);
	
	if(m_attached_items[1])
		m_attached_items[1]->RenderAttachableHud(root_object);
}
...
player_hud_motion* player_hud::find_motion_length(const shared_str& anim_name, const shared_str& hud_name)
{
	attachable_hud_item* pi			= create_hud_item(hud_name);
	player_hud_motion* anm = pi->m_hand_motions.find_motion(anim_name);
	if (!anm)
		return nullptr; // ms TEMPORARY
	//R_ASSERT2(anm,
	//	make_string("hudItem model [%s] has no motion with alias [%s]", hud_name.c_str(), anim_name.c_str()).c_str()
	//);
	return anm;
...
u32 player_hud::motion_length(const MotionID& M, const CMotionDef*& md, IKinematicsAnimated* model)
{
	...
	if (md->flags & esmStopAtEnd) 
	{
		...
		return				iFloor( 0.5f + 1000.f*motion->GetLength() / md->Dequantize(md->speed));
	}
	return					0;
}
...
u32 player_hud::anim_play(u16 part, const MotionID& M, BOOL bMixIn, const CMotionDef*& md, IKinematicsAnimated* model)
{
	...
	return motion_length(M, md, itemModel);
}
...
attachable_hud_item* player_hud::create_hud_item(const shared_str& sect)
{
	...
	res->LoadingHudItem(sect, m_model);
...
void player_hud::calc_transform(u16 attach_slot_idx, const Fmatrix& offset, Fmatrix& result, bool HandsCalc)
{
	if (m_model && HandsCalc)
	{
