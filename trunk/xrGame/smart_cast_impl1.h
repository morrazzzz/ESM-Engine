...
#pragma todo("Dima to all : this smart_cast is not optimized!")
	...
	struct CHelper2 {
		...
		IC	static T1* smart_cast(T2* p)
		{
			static_assert(!object_type_traits::is_const<T2>::value || object_type_traits::is_const<T1>::value, "Cannot use smart cast to convert const to non const");
			typedef typename object_type_traits::remove_const<T1>::type _T1;
			typedef typename  object_type_traits::remove_const<T2>::type _T2;
#ifdef DEBUG
