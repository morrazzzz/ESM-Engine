...
void CUISequenceSimpleItem::Start()
{
	...
	if (g_pGameLevel){
		...
		if(!stricmp(m_pda_section,"pda_contacts")){
		    CurrentGameUI()->PdaMenu().SetActiveSubdialog("button_pda_contacts");
			bShowPda = true;
		}
		if (!stricmp(m_pda_section, "pda_map")) {
			CurrentGameUI()->PdaMenu().SetActiveSubdialog("button_pda_map");
			bShowPda = true;
		}
		if (!stricmp(m_pda_section, "pda_quests")) {
			CurrentGameUI()->PdaMenu().SetActiveSubdialog("button_pda_quests");
			bShowPda = true;
		}
		if (!stricmp(m_pda_section, "pda_diary")) {
			CurrentGameUI()->PdaMenu().SetActiveSubdialog("button_pda_diary");
			bShowPda = true;
		}
		if (!stricmp(m_pda_section, "pda_ranking")) {
			CurrentGameUI()->PdaMenu().SetActiveSubdialog("button_pda_ranking");
			bShowPda = true;
		}
		if (!stricmp(m_pda_section, "pda_statistics")) {
			CurrentGameUI()->PdaMenu().SetActiveSubdialog("button_pda_statistics");
			bShowPda = true;
		}
		...
		if (!CurrentGameUI()->PdaMenu().IsShown() && bShowPda)
			CurrentGameUI()->PdaMenu().ShowDialog(true);
		else if (CurrentGameUI()->PdaMenu().IsShown() && !bShowPda)
...
bool CUISequenceSimpleItem::Stop			(bool bForce)
{
	...
	if (g_pGameLevel && CurrentGameUI() && (!m_pda_section || 0 == xr_strlen(m_pda_section))) {
		if (CurrentGameUI()->PdaMenu().IsShown())
			CurrentGameUI()->PdaMenu().HideDialog();
	}
