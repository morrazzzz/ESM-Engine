...
void CUIFixedScrollBar::InitScrollBar(Fvector2 pos, bool horizontal, LPCSTR profile)
{
	string256 _path;
	CUIXml xml_doc;
	xml_doc.Init(CONFIG_PATH, UI_PATH, "scroll_bar.xml");
...
{
	if(dik == MOUSE_LEFT_BUTTON_IDX && (last_hold_tm + m_hold_delay) < Device.dwTimeContinual)
	{
		if(OnMouseDownEx())
		{
...
{
	if(mouse_btn == MOUSE_LEFT_BUTTON_IDX && OnMouseDownEx())
		return true;
